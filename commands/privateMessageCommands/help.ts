...
const command: PrivateMessageCommand = async (message) => {
  ...
  const adminCallbackCommands = Object.entries(
    callbackConversationCommands
  ).filter(([, command]) => command.isAdminCommand);
  ...
  const userCallbackCommands = Object.entries(
    callbackConversationCommands
  ).filter(([, command]) => !command.isAdminCommand);

  const modesText = generateCommandList(
    "help_modes",
    [...Object.entries(modes), ...Object.entries(actionlessModes)],
    Object.entries(callbackModes)
  );
  const adminCommandsText = generateCommandList(
    "help_adminCommands",
    adminCommands,
    adminCallbackCommands
  );
  const userCommandsText = generateCommandList(
    "help_userCommands",
    userCommands,
    userCallbackCommands
  );
  const privateMessagesCommandsText = generateCommandList(
    "help_privateMessageCommands",
    Object.entries(privateMessageCommands)
  );
