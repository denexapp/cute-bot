...
const action: CallbackMode = async (
  message,
  settings,
  { callbackSecret, callbackServerChatId, callbackServerUrl, callbackServerUserId },
  botReacted,
  adminContext
) => {
  const { conversation_message_id: conversationMessageId, peer_id: peerId, from_id: fromId } = message;
