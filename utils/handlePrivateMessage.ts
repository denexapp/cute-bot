import { privateMessageCommands, callbackConversationCommands, conversationCommands, modes, callbackModes } from '../commands'
...
const handlePrivateMessage = async (message: Message, commandName: string) => {
  ...
  if (commandObject === undefined) {
    if (isConversationCommandWithNameExist(commandName)) {
      const text = phrase('common_commandAvailableInAGroupChatOnly', { commandName })
      await vk.messagesSend(peerId, text)
      return
    }
    if (isModeWithNameExist(commandName)) {
      const text = phrase('common_modeAvailableInAGroupChatOnly', { commandName })
      await vk.messagesSend(peerId, text)
      return
    }
