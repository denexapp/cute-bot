...
const editChatTemplateMessage = async (
  ...
  const success = await client.query(
    q.Let(
      ...
      q.If(
        q.Equals(q.Count(q.ToArray(q.Var("templates"))), 0),
        -1,
        q.If(
          q.Equals(q.Select(templateName, q.Var("templates"), -1), -1),
          -2,
          q.Do(
  ...
  const decodedNumber = decode(
    success,
    JsonDecoder.oneOf<0 | -1 | -2>(
      [
...
export default editChatTemplateMessage;
